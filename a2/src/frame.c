...
Frame_serialize	(	char *buffer, 
	...
	*(uint16_t *)&buffer[4 + length] = CRC16(buffer, length + 2 + sizeof(frame->index));

	return length + sizeof(frame->index) + 2 + 2;
}

frame_t * get_Head() {  return head; }

frame_t * get_Tail() {  return head; }

void set_Head( frame_t * frame) {   head = frame; }

void set_Tail( frame_t * frame) {   tail = frame; }
