...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		// since only one frame is received at any time it uses blocking function to receive from the client
		...
		if(expecting_frame == last_frame_received) {					
			if((Acutual_crc == calculated_crc) && probability_Calculate() /*user input*/){
				expecting_frame++;
