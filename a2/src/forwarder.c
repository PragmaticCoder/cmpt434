...
*/
void 
go_back_n_sliding(	struct sockaddr_in *client,
					char *buffer,
					int window_size	){
	...
	for(i = 0; (i < window_size) && (local_frame != NULL); i++){// loop till the window size or last frame
		int length = Frame_serialize(buffer, local_frame);		// convert the frame to linear buffer for sending via UDP
		if(length != 0) {
			if(sendto(	server, buffer, length, 0, (struct sockaddr *)client,
...
* if n = 1, every 10 messages 1 will be corrupted, acknowledgement will not be sent 
*/
...
int main(int argc, char* argv[]) {
	...
	check((argc >= 7),"Usage ./client <Hostname/IP Address> <DPort> <Window Size> <Timeout(S)> <RPort> <Probability(%%)>");
	...
	if(sigaction(SIGIO, &handler, 0) < 0){				// Signal Handler for SIGIO signal used for nonBlocking UDP socket
	...
	if(sigaction(SIGALRM, &handler, 0) < 0){			// signal handler for SIGALRM signal used for timeout purpose
	...
	if((socket_ID = socket(PF_INET, SOCK_DGRAM, 0)) < 0){	// open the socket for the server
		log_err("Failed to open socket");
	}
	...
	if(bind(socket_ID, (struct sockaddr *)&server, sizeof(struct sockaddr)) < 0 ){ // bind the server to the socket
		log_err("Failed to bind the socket");
	}

	if(fcntl(socket_ID, F_SETFL, O_NONBLOCK ) < 0){	// set receiver in non-blocking mode
		log_err("Failed to set the Socket in Async Mode");
	}
	...
	while(1){
		...
		if((receivedLength = recvfrom(socket_ID, buffer, (sizeof(buffer)/sizeof(buffer[0])) -1 ,
									  0, (struct sockaddr *)&client, &clientSize)) <= 0) {
			if((errno != EWOULDBLOCK) && (errno != EAGAIN)) {	// if no data byte found in the buffer
				log_err("Failed to Receive message from the server");
			}
		}
		else {
			last_frame_received = *(uint16_t *)buffer;			// index of the message received
			uint16_t message_length = *(uint16_t *)&buffer[2];	// length of the message received	
			uint16_t Acutual_crc	= *(uint16_t *)&buffer[4 + message_length];
			uint16_t calculated_crc = CRC16(buffer, 2 + sizeof(expecting_frame) + message_length);	// check for corruption in the communication

			buffer[4 + message_length] = '\0';
					
			debug("Ac CRC : 0x%04X Cal CRC : 0x%04X", Acutual_crc, calculated_crc);

			if(expecting_frame == last_frame_received) {		// check if received message is in proper sequence
				if((Acutual_crc == calculated_crc) && probability_Calculate() /*user input*/){		// simulate corruption/loss
					expecting_frame++;
		...
		// if the data have been received from the client , send acknowledgement back to the server
		if(receivedLength > 0) {
			*(uint16_t *)buffer = expecting_frame;
			if((sendto(	socket_ID, buffer, sizeof(expecting_frame), 0,
						(struct sockaddr *)&client, sizeof(struct sockaddr))) != sizeof(expecting_frame)){
		...
		if((current_frame != NULL) && !wait_for_reply ){	// if queue is not empty and not waiting for reply , send the message
