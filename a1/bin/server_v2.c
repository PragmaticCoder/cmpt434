...
HandleClient(int client)
{
  ...
  while (1) {
    ...
    if ((receivedLength =
           recv(client, receiveBuffer, sizeof(receiveBuffer) - 1, 0)) < 0) {
    ...
    if ((sentLength = send(client, receiveBuffer, strlen(receiveBuffer), 0)) <
...
main(int argc, char* argv[])
{
  ...
  while (1) {
    ...
    if ((clientFD =
           accept(serverFD, (struct sockaddr*)&clientSock, &clientSize)) < 0) {
